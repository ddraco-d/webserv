...
// 	{
// 		if (servers[i].getMoreInfo().count("root") == 0)
// 			std::cout << "NO ROOT!!!!\n";
// 		else if (dir_exists(servers[i].getMoreInfo()["root"].c_str()) != 1)
// 			std::cout << "NO VALID PATH!!!!" << servers[i].getMoreInfo()["root"].c_str() << "\n";
// 		else
// 			std::cout << "VALID PATH!!!!" << servers[i].getMoreInfo()["root"].c_str() << "\n";
...
// {
// 	int idx = info.find(":", 0);
// 	server->setHostString(std::string(info.begin(), info.begin()+idx));
// 	server->sethost(std::stoi(std::string(info.begin(), info.begin()+idx)));
// 	server->setPort(std::stoi(std::string(info.begin()+idx+1, info.end())));
// }
...
// 	server->setMoreInfo(more_info_init(info));
	
// 	if (server->getMoreInfo().count("listen"))
// 		server_init_listen(server->getMoreInfo()["listen"], server);
// 	else
...
// 		i = idx2;
// 	}
// 	server->getLocation()[loc.name] = loc;
// }
